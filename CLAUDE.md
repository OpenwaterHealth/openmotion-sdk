...
- **HIL note — console firmware wedge on abrupt USB cut:** when the console's USB is yanked mid-activity, the firmware command interface can stop responding (`No data received from UART within timeout`) until a **mains power cycle** (Shelly), even though the SDK reconnect logic is healthy (sensors recover fine, the monitor keeps cleanly retrying). This is a firmware bug — see `docs/superpowers/specs/2026-06-13-console-fw-host-loss-watchdog-spec.md`.
