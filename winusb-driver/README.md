...
and `pnputil /add-driver /install`s each INF. There is no manual, no-MSI
install path any more: the old `driver_install.cmd` added the cert to the
machine Root store, which the EV chain makes unnecessary.
...
- **Committed content:** the INFs, all four `.cat` files (libwdi/Zadig-generated
  driver catalogs — 3 sensor + `DFU_in_FS_Mode.cat` — committed **EV-signed**,
  byte-identical to the ones inside the canonical zip, so an EV dispatch
  re-signs none of them and spends one signing, on the MSI), `Product.wxs`,
  `build_driver_msi.ps1`, this README, the CI workflow,
  **and `OpenMotionDriver-x64.zip` — the canonical EV-signed artifact**
  ...
  A local or PR build overwrites the zip **and the four `.cat` files** in
  your working tree with legacy-signed ones — revert them; **never commit a
  legacy-signed zip or catalog**.
