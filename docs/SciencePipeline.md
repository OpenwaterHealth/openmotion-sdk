...
- **Dark frame protocol:** the firmware deterministically cuts laser illumination on a fixed schedule (see the frame-type table in §5.1). The pipeline never has to infer dark/light from the data.
...
Per `(side, cam_id)` pair, the stage maintains a `_FrameUnwrapper`. The side
...
1. **`step ≤ 0` (backward or duplicate)** → rejected as `"stale"`. Covers
...
|---|---|
| unwrapper rejected the frame (rules 1–2 above), or leading stale run | `"stale"` |
| `abs_id ≤ d` | `"warmup"` |
...
**File:** `omotion/pipeline/stages/timestamp_repair.py`. **Writes:** `quality`;
rewrites `timestamp_s` in place for flagged frames; inserts synthetic
`nan_filled` rows (the only case that rebuilds the batch); appends
`TimestampMisalignmentWindow` diagnostics events.

Sits after the raw tee so the raw CSV always records the *pre-repair* wire
capture (the tee snapshots for exactly this reason), and before the science
stages so everything downstream sees repaired time.
...
(distributed by frame-id count). A right-anchor candidate is only trusted
if its own timestamp is consistent with the left anchor at the nominal
cadence — otherwise a yet-to-be-flagged corrupt frame could anchor the
correction. With no usable right anchor the fallback is
`left_ts + gap × T`. Corrected rows get `quality = "ts_corrected"`.
...
event on the `"diagnostics"` channel (always — the scan DB summary must be
complete) and one WARNING log line, throttled to one per side per 2 s with
...
- **Normal mode** — one `session_data` row per per-camera `EnrichedCorrectedFrame` (`cam_id` 0..7, `side` 0/1, `frame_id` = absolute frame id) carrying `bfi`, `bvi`, `mean`, `contrast`, `temp`, `correction_status` (the frame's `quality` as a list; empty = clean) and `contact_quality` (see below). The stencilled leading dark frame of each interval is included, so the record is gapless at 40 Hz (except warmup frames 1..`discard_count` and the scan's terminal dark frame).
- **Reduced mode** — only the `cam_id=-1` side-average frames emitted by `SideAverageStage` are persisted; per-camera frames are skipped. Their `correction_status` is `SideAverageStage`'s camera-tagged list of every contributing camera's corrections (`l3:ts_corrected`), not a worst-wins flag.

**`contact_quality`** (bloodflow-app#589) — when the scan request attaches a `ContactQualityMonitor`, `ScanWorkflow` hands it to the sink as `cq_source`, and each row is stamped with `verdict_at(side, cam_id, frame_id)`: the verdict the monitor had latched *at that frame* (§11.3). Because `Tee("live")` is the last stage, within one batch the `"final"` interval reaches this sink before the monitor sees that batch's live frames, so the sink holds a row until `observed_through()` reaches its frame id, then stamps and buffers it (anything still held at scan end is stamped then). Side-average rows get the camera-tagged list of non-ok cameras. NULL without a monitor.
...
**Verdict history.** Every edge is also recorded against the `abs_frame_id` that caused it. `verdict_at(side, cam_id, frame_id)` returns the latched verdict as of that frame — `ok`, or the active reasons in fixed order (`poor_contact,ambient_light`) — and `None` for a camera outside the scan mask; `observed_through()` is the highest frame id consumed so far. `ScanDBSink` uses the pair to persist `contact_quality` per corrected row (§8.2). The history holds one entry per edge, so it stays small over a long scan.
