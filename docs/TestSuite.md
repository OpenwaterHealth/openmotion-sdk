...
| **3. Transport & protocol** | `MotionUart`, `CommInterface`, `StreamInterface`, `UartPacket`, `CommandError` | 18 | sw + hw | Adequate | `test_errors.py` (CRC, CommandError), `test_comm_paths.py` (path isolation, hw), `test_comm_transport_down.py` (cancellation, sw), `test_uart_cancel.py` (cancellable/bounded reads, sw). Hole: UART CRC-corruption + timeout HIL. |
...
| **— Connection lifecycle / hotplug** | `connection_monitor`, `connect_worker`, `connection_state` | — | sw + hw | Adequate (sw) | `test_connect_worker.py` (worker intents/coalesce/cooldown/abort-race, sw), `test_monitor_nonblocking.py` (failure-mode-A: one blocked connect can't starve another, sw), `test_connection_recovery.py` (HIL: cut/restore recovery, force-safe, mains cycle — combined-link cuts only on the current single-hub cabling). Hole: per-device-isolated cuts (needs re-cabling); the console-firmware wedge on abrupt cut is a firmware issue (see disconnect-hardening + console-fw specs). |
